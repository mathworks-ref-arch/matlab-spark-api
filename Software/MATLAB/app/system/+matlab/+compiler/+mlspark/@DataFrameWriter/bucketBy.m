function obj = bucketBy(obj, numBuckets, varargin)
    ...
    if ~isnumeric(numBuckets) || ~isscalar(numBuckets)
        error('SPARKAPI:dataframewriter_bucketby', ...
            'The numBuckets argument must be an integer scalar')
    end
    ...
    if length(cols) > 1  % multiple sort columns
        bucketCols = {cols(1), cols(2:end)};
    else  % a single sort column
        bucketCols = {cols, javaArray('java.lang.String',0)};
    end

    obj.dataFrameWriter.bucketBy(numBuckets, bucketCols{:});
